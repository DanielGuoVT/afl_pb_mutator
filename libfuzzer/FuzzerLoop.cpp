...
thread_localbool Fuzzer::IsMyThread;
...
struct MallocFreeTracer {
	...
	std::recursive_mutex TraceMutex;bool TraceDisabled = false;
};
...
void Fuzzer::AlarmCallback() {
	...
	size_t Seconds = duration_cast < seconds
...
void Fuzzer::PrintPulseAndReportSlowInput(const uint8_t *Data, size_t Size) {
	auto TimeOfUnit = duration_cast < seconds
...
bool Fuzzer::RunOne(const uint8_t *Data, size_t Size, bool MayDeleteFile,
	...
	if (NumNewFeatures) {
		...
		Corpus.AddToCorpus( { Data, Data + Size }, NumNewFeatures,
				MayDeleteFile, UniqFeatureSetTmp);
		return true;
	}
...
void Fuzzer::MutateAndTestOne() {
	...
	size_t CurrentMaxMutationLen = Min(MaxMutationLen,
	...
	for (int i = 0; i < Options.MutateDepth; i++) {
		...
		assert(
				NewSize <= CurrentMaxMutationLen && "Mutator return oversized unit");
		Size = NewSize;
...
void Fuzzer::PurgeAllocator() {
	...
	if (duration_cast < seconds
			> (system_clock::now() - LastAllocatorPurgeAttemptTime).count()
...
void Fuzzer::ReadAndExecuteSeedCorpora(const Vector<std::string> &CorpusDirs) {
	...
	for (auto &Dir : CorpusDirs) {
		...
		Printf("INFO: % 8zd files found in %s\n",
				SizedFiles.size() - LastNumFiles, Dir.c_str());
	...
	if (Options.MaxLen == 0)
		SetMaxInputLen(
				std::min(std::max(kMinDefaultLen, MaxSize), kMaxSaneLen));
	...
	if (SizedFiles.empty()) {
		...
		Printf("INFO: seed corpus: files: %zd min: %zdb max: %zdb total: %zdb"
				" rss: %zdMb\n", SizedFiles.size(), MinSize, MaxSize, TotalSize,
				GetPeakRSSMb());
	...
	while (true) {
		...
		if (Options.ExperimentalLenControl) {
			if (TmpMaxMutationLen < MaxMutationLen
				...
				TmpMaxMutationLen = Min(MaxMutationLen,
						TmpMaxMutationLen + Log(TmpMaxMutationLen));
				if (TmpMaxMutationLen <= MaxMutationLen)
...
void Fuzzer::MinimizeCrashLoop(const Unit &U) {
	...
	while (!TimedOut() && TotalNumberOfRuns < Options.MaxNumberOfRuns) {
		...
		for (int i = 0; i < Options.MutateDepth; i++) {
			size_t NewSize = MD.Mutate(CurrentUnitData, U.size(),
					MaxMutationLen);
		...
		if (Size != OtherSize || memcmp(Data, OtherData, Size) != 0) {
			...
			for (i = 0; i < Min(Size, OtherSize); i++)
				if (Data[i] != OtherData[i])
					break;
			Printf(
					"==%lu== ERROR: libFuzzer: equivalence-mismatch. Sizes: %zd %zd; "
